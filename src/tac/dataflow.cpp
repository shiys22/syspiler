...
void BasicBlock::analyzeLiveness(void) {
	...
	for (t = tac_chain; t != NULL; t = t->next) pnt++;
	Tac *S[pnt];
	pnt = 0;
	for (t = tac_chain; t != NULL; t = t->next)
	{
		if (t -> op_code == Tac::PARAM || t -> op_code == Tac::CALL)
			S[pnt++] = t;
	}
	...
	for (t = t->prev; t != NULL; t = t->prev) {
		...
		{
			pnt--;
			while (pnt != -1 && S[pnt] -> op_code == Tac::PARAM)
				t->LiveOut->add(S[pnt--]->op0.var);
			break;
		}
		case Tac::PARAM:
			// haha
