...
class SemPass2 : public ast::Visitor {
	...
	virtual void visit(ast::ForExprStmt *);
	virtual void visit(ast::ForDeclStmt *);
 ...
 */
void SemPass2::visit(ast::Call *c)
{
	for (ast::ExprList::iterator it = c->exprs->begin();
		 it != c->exprs->end(); ++it)
		(*it)->accept(this);
	Symbol *v = scopes ->lookup(c->name, c->getLocation());
	if (NULL == v) {
		issue(c->getLocation(), new SymbolNotFoundError(c->name));
		goto issue_error_type;
	} else if (!v->isFunction()) {
		issue(c->getLocation(), new NotVariableError(v));
		goto issue_error_type;
	} else if (dynamic_cast<Function *>(v) -> getParamCount() != c -> exprs -> length()) {
		issue(c->getLocation(), new BadArgCountError((Function *)v));
		goto issue_error_type;
	} else {
		c->ATTR(type) = BaseType::Int;
		c->ATTR(sym) = (Function *)v;
	}
...
void SemPass2::visit(ast::VarRef *ref) {
	...
	if (NULL == v) {
		...
		ref -> is_global = v -> getScope() == scopes -> bottom();
...
void SemPass2::visit(ast::ForExprStmt *s) {
...
void SemPass2::visit(ast::ForDeclStmt *s) {
	scopes->open(s->ATTR(scope));
...
void SemPass2::visit(ast::Program *p) {
	std::cerr << "Starting SemPass2" << std::endl;
