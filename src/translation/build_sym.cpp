...
void SemPass1::visit(ast::FuncDefn *fdef) {
	...
	Function *f = new Function(fdef->name, t, fdef->getLocation(), fdef -> formals -> length());
	fdef->ATTR(sym) = f;
	...
	if (NULL != sym)
	{
		if (sym -> isFunction() && !(dynamic_cast<Function*>(sym) -> isDefined() && fdef -> forward_decl))
			dynamic_cast<Function*>(sym) -> tryDefine(!fdef -> forward_decl);
		else issue(fdef->getLocation(), new DeclConflictError(fdef->name, sym));
	}
	else
		scopes->declare(f);
...
void SemPass1::visit(ast::ForExprStmt *s) {
...
void SemPass1::visit(ast::ForDeclStmt *s) {
	Scope *scope = new LocalScope();
	s->ATTR(scope) = scope;
	scopes->open(scope);
...
void SemPass1::visit(ast::VarDecl *vdecl) {
	...
	{
		scopes -> declare(v);
		if (scopes -> bottom() == scopes -> top())
		{
			vdecl -> is_global = true;
			if (vdecl -> init == NULL) v -> setGlobalInit(0);
			else
			{
				assert(dynamic_cast<ast::IntConst*>(vdecl -> init) != NULL);
				v -> setGlobalInit(dynamic_cast<ast::IntConst*>(vdecl -> init) -> value);
			}
		}
	}
