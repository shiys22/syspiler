...
RiscvDesc::RiscvDesc(void) {
	...
	_reg[RiscvReg::T6] = new RiscvReg("t6", false); // ok swap
...
void RiscvDesc::emitPieces(scope::GlobalScope *gscope, Piece *ps,
	...
	_result = &os;
	std::cerr << "TAC2RISC_V" << std::endl;
	// output to .data and .bss segment
	std::ostringstream _data, _bss;

	if (Option::getLevel() == Option::ASMGEN) {
		emit(EMPTY_STR, ".data", NULL);
		for (scope::ScopeIterator it = gscope -> begin(); it != gscope -> end(); it++)
			if ((*it) -> isVariable())
			{
				emit(EMPTY_STR, (".globl " + (*it) -> getName()).c_str(), NULL);
				emit((*it) -> getName().c_str(), NULL, NULL);
				emit(EMPTY_STR, (".word " + std::to_string(dynamic_cast<symb::Variable*>(*it) -> getGlobalInit())).c_str(), NULL);
			}
		// program preamble
...
RiscvInstr *RiscvDesc::prepareSingleChain(BasicBlock *b, FlowGraph *g) {
	...
	case BasicBlock::BY_JUMP:
		...
		addInstr(RiscvInstr::J, NULL, NULL, NULL, 0, std::string(g->getBlock(b->next[0])->entry_label), NULL);
		// "B" for "branch"
		break;
	...
	case BasicBlock::BY_JZERO:
		...
		addInstr(RiscvInstr::BEQZ, _reg[r0], NULL, NULL, 0, std::string(g->getBlock(b->next[0])->entry_label), NULL);
		addInstr(RiscvInstr::J, NULL, NULL, NULL, 0, std::string(g->getBlock(b->next[1])->entry_label), NULL);
		break;

	case BasicBlock::BY_RETURN:
		r0 = getRegForRead(b->var, 0, b->LiveOut);
		spillDirtyRegs(b->LiveOut); // just to deattach all temporary variables
		addInstr(RiscvInstr::MOVE, _reg[RiscvReg::A0], _reg[r0], NULL, 0, EMPTY_STR, NULL);
		addInstr(RiscvInstr::MOVE, _reg[RiscvReg::SP], _reg[RiscvReg::FP], NULL, 0, EMPTY_STR, NULL);
		addInstr(RiscvInstr::LW, _reg[RiscvReg::RA], _reg[RiscvReg::FP], NULL, -4, EMPTY_STR, NULL);
		addInstr(RiscvInstr::LW, _reg[RiscvReg::FP], _reg[RiscvReg::FP], NULL, -8, EMPTY_STR, NULL);
...
void RiscvDesc::emitTac(Tac *t) {
	...
	t->dump(oss);
	addInstr(RiscvInstr::COMMENT, NULL, NULL, NULL, 0, EMPTY_STR, oss.str().c_str());
	...
	case Tac::ASSIGN:
		...
		break;
	case Tac::RETURN:
		spillReg(RiscvReg::A0, t -> LiveOut);
		addInstr(RiscvInstr::MOVE, _reg[RiscvReg::A0], _reg[getRegForRead(t->op0.var, RiscvReg::A0, t->LiveOut)], NULL, 0, EMPTY_STR, NULL);
		break;
	case Tac::PARAM:
		passParamReg(t);
		break;
	...
	case Tac::CALL:
		...
		addInstr(RiscvInstr::MOVE, _reg[getRegForWrite(t->op0.var, RiscvReg::A0, 0, t->LiveOut)], _reg[RiscvReg::A0], NULL, 0, EMPTY_STR, NULL);
		break;
...
void RiscvDesc::emitCallTac(RiscvInstr::OpCode op, Tac *t) {
	...
	addInstr(op, _reg[r0], NULL, NULL, 0, "_" + t -> op1.label -> str_form, NULL);
}
 ...
 */
void RiscvDesc::passParamReg(Tac *t) {
	std::ostringstream oss;
	int i = lookupReg(t->op0.var);
	// RISC-V use a0-a7 to pass the first 8 parameters, so it's ok to do so.
	if (t -> op1.ival < 8)
	{
		spillReg(RiscvReg::A0 + t -> op1.ival, t->LiveOut);
		if(i < 0) {
			auto v = t->op0.var;
			RiscvReg *base = _reg[RiscvReg::FP];
			oss << "load " << v << " from (" << base->name
				<< (v->offset < 0 ? "" : "+") << v->offset << ") into "
				<< _reg[RiscvReg::A0 + t -> op1.ival]->name;
			addInstr(RiscvInstr::LW, _reg[RiscvReg::A0 + t -> op1.ival], base, NULL, v->offset, EMPTY_STR, oss.str().c_str());
		} else {
			addInstr(RiscvInstr::MOVE, _reg[RiscvReg::A0 + t -> op1.ival], _reg[i], NULL, 0, EMPTY_STR, NULL);
		}
		_reg[RiscvReg::A0 + t -> op1.ival] -> var = t -> op0.var;
	}
	else
	{
		auto v = t->op0.var;
		if(i < 0) {
			addInstr(RiscvInstr::LW, _reg[RiscvReg::T6], _reg[RiscvReg::FP], NULL, v->offset, EMPTY_STR, NULL);
			_frame -> temp_reserve(v);
			addInstr(RiscvInstr::SW, _reg[RiscvReg::T6], _reg[RiscvReg::FP], NULL, v->offset, EMPTY_STR, NULL);
		} else
		{
			_frame -> temp_reserve(v);
			addInstr(RiscvInstr::SW, _reg[i], _reg[RiscvReg::FP], NULL, v->offset, EMPTY_STR, NULL);
		}
	}
}
 ...
 *   cnt   - reg offset A0 + cnt
 */
void RiscvDesc::getParamReg(Tac *t) {
	if (t -> op1.ival < 8)
	{
		_reg[RiscvReg::A0 + t -> op1.ival]->var = t->op0.var;
		_reg[RiscvReg::A0 + t -> op1.ival]->dirty = true;
	}
	else addInstr(RiscvInstr::LW, _reg[getRegForWrite(t -> op0.var, 0, 0, t -> LiveOut)], _reg[RiscvReg::FP], NULL, (t -> op1.ival - 8) * 4, EMPTY_STR, NULL);
}
...
//		emit(EMPTY_STR, NULL, i->comment);
	...
	case RiscvInstr::LAND: {
		std::string tmp = RiscvDesc::getNewLabel();
		oss << "beq" << i->r1->name << ",zero," << tmp;
		emit(EMPTY_STR, oss.str().c_str(), NULL);
		oss.str("");
		oss << std::left << std::setw(6);
		oss << "snez " << i->r0->name << "," << i->r2->name;
		emit(EMPTY_STR, oss.str().c_str(), NULL);
		oss.str("");
		oss << tmp << ":" << std::endl;
		break;
	...
	case RiscvInstr::SW2:
		oss << "sw" << i -> r0 -> name << ", " << i -> l << ", " << i -> r1 -> name;
		break;
...
void RiscvDesc::addInstr(RiscvInstr::OpCode op_code, RiscvReg *r0, RiscvReg *r1,
	...
	_tail->i = i;
	_tail->l = l;
	if (NULL != cmt && strlen(cmt) == 2) cmt = NULL;
	_tail->comment = cmt;
}
...
int RiscvDesc::getRegForRead(Temp v, int avoid1, LiveSet *live) {
	...
	if (i < 0) {
		...
		if (v -> is_global)
		{
			addInstr(RiscvInstr::LA, _reg[i], NULL, NULL, 0, v -> str, NULL);
			oss << "read " << v << " from heap";
			addInstr(RiscvInstr::LW, _reg[i], _reg[i], NULL, 0, EMPTY_STR, oss.str().c_str());
		}
		else if (v->is_offset_fixed) {
			RiscvReg *base = _reg[RiscvReg::FP];
			oss << "load " << v << " from (" << base->name
				<< (v->offset < 0 ? "" : "+") << v->offset << ") into "
				<< _reg[i]->name;
	...
	if ((NULL != v) && _reg[i]->dirty && live->contains(v)) {
		...
		if (v -> is_global)
		{
			oss << "spill " << v << " from " << _reg[i]->name << " to "
				<< v -> str << " using swap temp T6 (sorry T6!!)";
			addInstr(RiscvInstr::SW2, _reg[i], _reg[RiscvReg::T6], NULL, 0, v -> str, oss.str().c_str());
		}
		else
		{
			oss << "spill " << v << " from " << _reg[i]->name << " to ("
				<< base->name << (v->offset < 0 ? "" : "+") << v->offset << ")";
			addInstr(RiscvInstr::SW, _reg[i], base, NULL, v->offset, EMPTY_STR, oss.str().c_str());
		}
	}
